...
namespace Tips.Purchase.Api.Repository
{
    public class PurchaseOrderRepository : RepositoryBase<PurchaseOrder>, IPurchaseOrderRepository
    {
        ...
        public async Task<List<PurchaseOrder>> GetAllUnApprovedLastestPOsbyProcurementType(string ProcurementType)
        {
            var Pos = FindByCondition(x => x.InApproval == false && x.ProcurementType == ProcurementType && x.RevisionNumber == (_tipsPurchaseDbContext.PurchaseOrders.Where(inv => inv.PONumber == x.PONumber).OrderByDescending(i => i.RevisionNumber).Select(i => i.RevisionNumber).First()))
                .Include(t => t.POItems)
        ...
        public async Task<List<PONameList>> GetAllActivePurchaseOrderNameList()
        {
            List<PONameList> activePurchaseOrderNameList = await _tipsPurchaseDbContext.PurchaseOrders
            .GroupBy(x => x.PONumber)
            .Select(g => new PONameList()
            {
        ...
        public async Task<PagedList<PurchaseOrder>> GetAllLastestPendingPOApprovalIList([FromQuery] PagingParameter pagingParameter, [FromQuery] SearchParamess searchParams)
        {
            ...
            && ((x.POApprovalI == false && x.IsDeleted == false && x.IsModified == false && x.PoStatus != PoStatus.ShortClosed)
        ...
        public async Task<PagedList<PurchaseOrder>> GetAllLastestPendingPOApprovalIIList([FromQuery] PagingParameter pagingParameter, [FromQuery] SearchParamess searchParams)
        {
            ...
            x.VendorId.Equals(searchParams.SearchValue) || x.VendorAddress.Equals(searchParams.SearchValue) || x.POApprovedIBy.Equals(searchParams.SearchValue)
            || x.POApprovedIIBy.Equals(searchParams.SearchValue) || x.ProcurementType.Equals(searchParams.SearchValue))
            && ((x.POApprovalI == true && x.POApprovalII == false && x.IsDeleted == false && x.IsModified == false
            && x.PoStatus != PoStatus.ShortClosed) && (x.RevisionNumber == _tipsPurchaseDbContext.PurchaseOrders.Where(r => r.PONumber == x.PONumber).Max(r => r.RevisionNumber))))
                .OrderByDescending(x => x.Id);
        ...
        public async Task<PagedList<PurchaseOrder>> GetAllLastestPendingPOApprovalIIIListForAvision([FromQuery] PagingParameter pagingParameter, [FromQuery] SearchParamess searchParams)
        {
            ...
            && ((x.POApprovalI == true && x.POApprovalII == true && x.POApprovalIII == false && x.ApprovalCount >= 3 && x.IsDeleted == false && x.IsModified == false
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllPendingPOApprovalINameList()
        {
            var pendingPOApprovalINameList = await _tipsPurchaseDbContext.PurchaseOrders
                .Where(x => x.POApprovalI == false && x.POApprovalII == false && x.IsModified == false && x.IsDeleted == false && (x.PoStatus != PoStatus.ShortClosed && x.PoStatus != PoStatus.Closed))
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllPendingPOApprovalIINameList()
        {
            ...
            .Where(x => x.POApprovalII == false && x.POApprovalI == true && x.IsDeleted == false && x.IsModified == false && (x.PoStatus != PoStatus.ShortClosed && x.PoStatus != PoStatus.Closed))
            .GroupBy(x => x.PONumber)
            .Select(g => new PurchaseOrderIdNameListDto()
            {
                Id = g.OrderByDescending(x => x.RevisionNumber).FirstOrDefault().Id,
        ...
        public async Task<PurchaseOrder> GetLatestPurchaseOrderItemsByPONumber(string poNumber)
        {
            var purchaseOrderDetailbyPONumber = await _tipsPurchaseDbContext.PurchaseOrders
        ...
        public async Task<List<PurchaseOrder>> GetAllPurchaseOrderbyPurchaseOrderNumber(string poNumber)
        {
            var purchaseOrderDetail = await _tipsPurchaseDbContext.PurchaseOrders
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllServicePoNumberListByVendorId(string vendorId)
        {
            PoStatus[] status = { PoStatus.Open, PoStatus.PartiallyClosed };

            IEnumerable<PurchaseOrderIdNameListDto> pONameListbyVendorId = await _tipsPurchaseDbContext.PurchaseOrders
                           .Where(x => x.VendorId == vendorId && status.Contains(x.PoStatus) && x.POApprovalII == true && x.ProcurementType == "Service")
                           .Select(x => new PurchaseOrderIdNameListDto()
                           {
                               Id = x.Id,
                               PONumber = x.PONumber
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllNonServicePoNumberListByVendorId(string vendorId)
        {
            PoStatus[] status = { PoStatus.Open, PoStatus.PartiallyClosed };
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetKIT_PoNumberListByVendorId(string vendorId)
        {
            PoStatus[] status = { PoStatus.Open, PoStatus.PartiallyClosed };
            ...
            IEnumerable<PurchaseOrderIdNameListDto> pONameListbyVendorId = await _tipsPurchaseDbContext.PurchaseOrders
                           ...
                           && x.ProcurementType != "Service" && x.PoType == PoType.Kit)
                           .Select(x => new PurchaseOrderIdNameListDto()
                           {
                               Id = x.Id,
                               PONumber = x.PONumber
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllServicePoNumberListByVendorIdForAvision(string vendorId)
        {
            PoStatus[] status = { PoStatus.Open, PoStatus.PartiallyClosed };
        ...
        public async Task<IEnumerable<PurchaseOrderIdNameListDto>> GetAllNonServicePoNumberListByVendorIdForAvision(string vendorId)
        {
            PoStatus[] status = { PoStatus.Open, PoStatus.PartiallyClosed };
        ...
        public async Task<IEnumerable<poconfirmation_report_with_pagination_Dto>> GetPoConfirmationSPReportwithParam(string? ItemNumber,string? ProjectNumber, string? PONumber, string? VendorName, string? POStatus
                                                                                                       , string? Approval, string? RecordType)
        {
            var poconfirmmation = await _tipsPurchaseDbContext.Set<poconfirmation_report_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL poconfirmation_report_with_parameters({ItemNumber},{ProjectNumber},{PONumber},{VendorName},{POStatus},{Approval},{RecordType})")
                    .AsNoTracking()
                    .ToListAsync();

            return poconfirmmation;
        }
        public async Task<IEnumerable<poconfirmation_report_with_pagination_Dto>> GetPoConfirmationLimitSPReportwithParam(string? ItemNumber,string? ProjectNumber, string? PONumber, string? VendorName, string? POStatus
                                                                                                      , string? Approval, string? RecordType, int? Offset, int? Limit)
        {
            var poconfirmmation = await _tipsPurchaseDbContext.Set<poconfirmation_report_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL poconfirmation_report_with_parameters_with_pagination({ItemNumber},{ProjectNumber},{PONumber},{VendorName},{POStatus},{Approval},{RecordType},{Offset},{Limit})")
                    .AsNoTracking()
                    .ToListAsync();

            return poconfirmmation;
        }
        public async Task<IEnumerable<poconfirmation_report_with_pagination_Dto>> GetPoConfirmationSPReportwithDate(DateTime? FromDate, DateTime? ToDate, string? Approval, string? RecordType)
        {
            var poconfirmmation = await _tipsPurchaseDbContext.Set<poconfirmation_report_with_pagination_Dto>()
        ...
        public async Task<IEnumerable<poconfirmation_report_with_pagination_Dto>> GetPoConfirmationLimitSPReportwithDate(DateTime? FromDate, DateTime? ToDate, string? Approval,
                                                                                                                            string? RecordType, int? Offset, int? Limit)
        {
            var poconfirmmation = await _tipsPurchaseDbContext.Set<poconfirmation_report_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL poconfirmation_report_withdate_with_pagination({FromDate},{ToDate},{Approval},{RecordType},{Offset},{Limit})")
                    .ToListAsync();

            return poconfirmmation;
        }

        public async Task<IEnumerable<podeliveryschedule_report_with_parameters_with_pagination_Dto>> GetPoDeliverySchedulewithParam(string? ItemNumber,string? ProjectNumber,string? PONumber, string? VendorName, string? POStatus
                                                                                                       , string? Approval, string? RecordType)
        {
            var podeliveryschedule = await _tipsPurchaseDbContext.Set<podeliveryschedule_report_with_parameters_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL podeliveryschedule_report_with_parameters({ItemNumber},{ProjectNumber},{PONumber},{VendorName},{POStatus},{Approval},{RecordType})")
                    .ToListAsync();

            return podeliveryschedule;
        }
        public async Task<IEnumerable<podeliveryschedule_report_with_parameters_with_pagination_Dto>> GetPoDeliveryScheduleLimitwithParam(string? ItemNumber, string? ProjectNumber, string? PONumber, string? VendorName, string? POStatus
                                                                                                       , string? Approval, string? RecordType, int? Offset, int? Limit)
        {
            var podeliveryschedule = await _tipsPurchaseDbContext.Set<podeliveryschedule_report_with_parameters_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL podeliveryschedule_report_with_parameters_with_pagination({ItemNumber},{ProjectNumber},{PONumber},{VendorName},{POStatus},{Approval},{RecordType},{Offset},{Limit})")
                    .ToListAsync();
        ...
        public async Task<IEnumerable<podeliveryschedule_report_with_parameters_with_pagination_Dto>> GetPoDeliveryScheduleSPReportwithDate(DateTime? FromDate, DateTime? ToDate, string? Approval, string? RecordType)
        {
            var podeliveryschedule = await _tipsPurchaseDbContext.Set<podeliveryschedule_report_with_parameters_with_pagination_Dto>()
        ...
        public async Task<IEnumerable<podeliveryschedule_report_with_parameters_with_pagination_Dto>> GetPoDeliveryScheduleLimitSPReportwithDate(DateTime? FromDate, DateTime? ToDate, string? Approval,
                                                                                                            string? RecordType, int? Offset, int? Limit)
        {
            var podeliveryschedule = await _tipsPurchaseDbContext.Set<podeliveryschedule_report_with_parameters_with_pagination_Dto>()
                    .FromSqlInterpolated($"CALL podeliveryschedule_report_withdate_with_pagination({FromDate},{ToDate},{Approval},{RecordType},{Offset},{Limit})")
                    .ToListAsync();
        ...
        public async Task<IEnumerable<PoProjectSPReport>> GetPoProjectLimitSPReportwithParam(string? ItemNumber, string? PONumber, string? VendorName, string? POStatus
                                                                                                       , string? Approval, string? ProjectNumber, string? RecordType
                                                                                                , int? Offset, int? Limit)
        {
            var poproject = await _tipsPurchaseDbContext.Set<PoProjectSPReport>()
                    .FromSqlInterpolated($"CALL poproject_report_with_parameters_with_pagination({ItemNumber},{PONumber},{VendorName},{POStatus},{Approval},{ProjectNumber},{RecordType},{Offset},{Limit})")
                    .AsNoTracking()
                    .ToListAsync();
        ...
        public async Task<IEnumerable<PoProjectSPReport>> GetPoProjectLimitSPReportwithDate(DateTime? FromDate, DateTime? ToDate, string? Approval, string? RecordType
                                                                                                    , int? Offset, int? Limit)
        {
            var poproject = await _tipsPurchaseDbContext.Set<PoProjectSPReport>()
                            .FromSqlInterpolated($"CALL poproject_report_withdate_with_pagination({FromDate},{ToDate},{Approval},{RecordType},{Offset},{Limit})")
                            .AsNoTracking()
                            .ToListAsync();
    ...
    public class PurchaseOrderItemRepository : RepositoryBase<PoItem>, IPoItemsRepository
    {
        ...
        public async Task<PoItem> GetPoItemDetailsByPONumberandItemNo(string ItemNumber, string PONumber, int poItemId)
        {
            var getPODetailsByPONOandItemNo = await _tipsPurchaseDbContexts.PoItems
