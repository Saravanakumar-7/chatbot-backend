...
namespace Tips.Warehouse.Api.Controllers
{
    ...
    public class LocationTransferController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> InventoryWithLocationtransferUnitPriceSPReportDatesAvi([FromQuery] DateTime? FromDate, [FromQuery] DateTime? ToDate)
        {
            ServiceResponse<IEnumerable<InvLocationtransferUnitPriceDateSpForAvi>> serviceResponse = new ServiceResponse<IEnumerable<InvLocationtransferUnitPriceDateSpForAvi>>();
            try
            {
                var products = await _locationTransferRepository.InventoryWithLocationtransferUnitPriceSPReportDatesAvi(FromDate, ToDate);

                if (products == null)
                {
                    serviceResponse.Data = null;
                    serviceResponse.Message = $"InvLocationtransferUnitPriceDateSpForAvi hasn't been found.";
                    serviceResponse.Success = false;
                    serviceResponse.StatusCode = HttpStatusCode.NotFound;
                    _logger.LogError($"InvLocationtransferUnitPriceDateSpForAvi hasn't been found in db.");
                    return NotFound(serviceResponse);
                }
                else
                {
                    serviceResponse.Data = products;
                    serviceResponse.Message = "Returned InvLocationtransferUnitPriceDateSpForAvi Details";
                    serviceResponse.Success = true;
                    serviceResponse.StatusCode = HttpStatusCode.OK;
                    return Ok(serviceResponse);
                }
            ...
            {
                _logger.LogError($"Error Occured in InventoryWithLocationtransferUnitPriceSPReportDatesAvi API : \n {ex.Message} \n{ex.InnerException}");
                serviceResponse.Data = null;
                serviceResponse.Message = $"Error Occured in InventoryWithLocationtransferUnitPriceSPReportDatesAvi API : \n {ex.Message}";
                serviceResponse.Success = false;
                serviceResponse.StatusCode = HttpStatusCode.InternalServerError;
                return StatusCode(500, serviceResponse);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> InventoryWithLocationtransferUnitPriceSPReportWithParamAvi([FromBody] InventoryFilterParams inventoryFilterParams)
        {
            ServiceResponse<IEnumerable<InvLocationtransferUnitPriceSpWithParamForAvi>> serviceResponse = new ServiceResponse<IEnumerable<InvLocationtransferUnitPriceSpWithParamForAvi>>();
            try
            {
                var products = await _locationTransferRepository.InventoryWithLocationtransferUnitPriceSPReportWithParamAvi(inventoryFilterParams.PartNumber, inventoryFilterParams.Description, inventoryFilterParams.Warehouse,
                                                                                                inventoryFilterParams.Location, inventoryFilterParams.ProjectNumber);
                if (products == null)
                {
                    serviceResponse.Data = null;
                    serviceResponse.Message = $"InvLocationtransferUnitPriceSpWithParamForAvi hasn't been found.";
                    serviceResponse.Success = false;
                    serviceResponse.StatusCode = HttpStatusCode.NotFound;
                    _logger.LogError($"InvLocationtransferUnitPriceSpWithParamForAvi hasn't been found in db.");
                    return NotFound(serviceResponse);
                }
                else
                {
                    serviceResponse.Data = products;
                    serviceResponse.Message = "Returned InvLocationtransferUnitPriceSpWithParamForAvi Details";
                    serviceResponse.Success = true;
                    serviceResponse.StatusCode = HttpStatusCode.OK;
                    return Ok(serviceResponse);
                }
            ...
            {
                _logger.LogError($"Error Occured in InventoryWithLocationtransferUnitPriceSPReportWithParamAvi API : \n {ex.Message} \n{ex.InnerException}");
                serviceResponse.Data = null;
                serviceResponse.Message = $"Error Occured in InventoryWithLocationtransferUnitPriceSPReportWithParamAvi API : \n {ex.Message}";
                serviceResponse.Success = false;
                serviceResponse.StatusCode = HttpStatusCode.InternalServerError;
                return StatusCode(500, serviceResponse);
            }
