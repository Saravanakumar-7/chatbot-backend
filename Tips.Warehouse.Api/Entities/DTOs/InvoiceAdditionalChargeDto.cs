using System.ComponentModel.DataAnnotations;
using Microsoft.EntityFrameworkCore;

namespace Tips.SalesService.Api.Entities.DTOs
{
    public class InvoiceAdditionalChargesDto
    {
        [Key]
        public int Id { get; set; }
        public int? SalesOrderId { get; set; }
        public String? DONumber { get; set; }
        public string? AdditionalChargesLabelName { get; set; }
        public string? AddtionalChargesValueType { get; set; }

        [Precision(18, 3)]
        public decimal? AddtionalChargesValueAmount { get; set; }

        [Precision(13, 3)]
        public decimal? TotalValue { get; set; }

        [Precision(13, 3)]
        public decimal? InvoicedValue { get; set; }

        [Precision(18, 3)]
        public decimal? IGST { get; set; }

        [Precision(18, 3)]
        public decimal? CGST { get; set; }

        [Precision(18, 3)]
        public decimal? UTGST { get; set; }

        [Precision(18, 3)]
        public decimal? SGST { get; set; }
        public int SalesAdditionalChargeId { get; set; }

    }

    public class InvoiceAdditionalChargesPostDto
    {
        public int? SalesOrderId { get; set; }
        public String? DONumber { get; set; }
        public string? AdditionalChargesLabelName { get; set; }
        public string? AddtionalChargesValueType { get; set; }

        [Precision(18, 3)]
        public decimal? AddtionalChargesValueAmount { get; set; }

        [Precision(13, 3)]
        public decimal? TotalValue { get; set; }

        [Precision(13, 3)]
        public decimal? InvoicedValue { get; set; }

        [Precision(18, 3)]
        public decimal? IGST { get; set; }

        [Precision(18, 3)]
        public decimal? CGST { get; set; }

        [Precision(18, 3)]
        public decimal? UTGST { get; set; }

        [Precision(18, 3)]
        public decimal? SGST { get; set; }
        public int SalesAdditionalChargeId { get; set; }

    }
    public class InvoiceAdditionalChargesUpdateDto
    {
        public int? SalesOrderId { get; set; }
        public String? DONumber { get; set; }
        public string? AdditionalChargesLabelName { get; set; }
        public string? AddtionalChargesValueType { get; set; }

        [Precision(18, 3)]
        public decimal? AddtionalChargesValueAmount { get; set; }

        [Precision(13, 3)]
        public decimal? TotalValue { get; set; }

        [Precision(13, 3)]
        public decimal? InvoicedValue { get; set; }

        [Precision(18, 3)]
        public decimal? IGST { get; set; }

        [Precision(18, 3)]
        public decimal? CGST { get; set; }

        [Precision(18, 3)]
        public decimal? UTGST { get; set; }

        [Precision(18, 3)]
        public decimal? SGST { get; set; }
        public int SalesAdditionalChargeId { get; set; }

    }
    public class InvoiceAdditionalChargesReportDto
    {
        [Key]
        public int Id { get; set; }
        public int? SalesOrderId { get; set; }
        public string? InvoiceNumber { get; set; }
        public String? DONumber { get; set; }
        public string? AdditionalChargesLabelName { get; set; }
        public string? AddtionalChargesValueType { get; set; }

        [Precision(18, 3)]
        public decimal? AddtionalChargesValueAmount { get; set; }

        [Precision(13, 3)]
        public decimal? TotalValue { get; set; }

        [Precision(13, 3)]
        public decimal? InvoicedValue { get; set; }

        [Precision(18, 3)]
        public decimal? IGST { get; set; }

        [Precision(18, 3)]
        public decimal? CGST { get; set; }

        [Precision(18, 3)]
        public decimal? UTGST { get; set; }

        [Precision(18, 3)]
        public decimal? SGST { get; set; }
        public int SalesAdditionalChargeId { get; set; }

    }
    public class DoNoInvoiceAdditionalChargesDto
    {
        [Key]
        public int Id { get; set; }
        public int? SalesOrderId { get; set; }
        public String? DONumber { get; set; }
        public string? AdditionalChargesLabelName { get; set; }
        public string? AddtionalChargesValueType { get; set; }

        [Precision(18, 3)]
        public decimal? AddtionalChargesValueAmount { get; set; }

        [Precision(13, 3)]
        public decimal? TotalValue { get; set; }

        [Precision(13, 3)]
        public decimal? InvoicedValue { get; set; }

        [Precision(18, 3)]
        public decimal? IGST { get; set; }

        [Precision(18, 3)]
        public decimal? CGST { get; set; }

        [Precision(18, 3)]
        public decimal? UTGST { get; set; }

        [Precision(18, 3)]
        public decimal? SGST { get; set; }
        public int SalesAdditionalChargeId { get; set; }

    }
}