...
namespace Tips.Warehouse.Api.Repository
{
    ...
    .ToList<ConsumptionReport>();
        ...
        public async Task<List<InventoryQtyforDO>> GetInventorybyItem(string itemNumber)
        {
            ...
            var invdetails = await FindAll().Where(x => x.PartNumber == itemNumber && !skipWareHouse.Contains(x.Warehouse) && x.Balance_Quantity > 0)
                .Select(x => new GetInventoryQtyforDO()
                {
                    Warehouse = x.Warehouse,
                    Location = x.Location,
        ...
        {
            var getInventoryDetailsById = await _tipsWarehouseDbContext.Inventories.Where(x => x.GrinNo == GrinNo && x.Warehouse == "IQC" && x.Location == "IQC" &&
                                        x.GrinPartId == GrinPartsId && x.PartNumber == ItemNumber && x.IsStockAvailable == true)
                          .ToListAsync();
        ...
        public async Task<IEnumerable<Inventory>> GetInventoryDetailsByItemNumberandLocation(string ItemNumber, string Location, string Warehouse, string projectNumber,string lotnumber)

        {
            var getInventoryDetailsByItemAndLoc = await _tipsWarehouseDbContext.Inventories
                .Where(x => x.PartNumber == ItemNumber && x.ProjectNumber == projectNumber && x.Location == Location && x.Warehouse == Warehouse && x.LotNumber == lotnumber && x.IsStockAvailable == true).ToListAsync();

            return getInventoryDetailsByItemAndLoc;
        }

        public async Task<IEnumerable<Inventory>> GetInventoryDetailsByItemNumberandLotNumber(string ItemNumber, string projectNumber, string lotNumber)

        {
            var getInventoryDetailsByItemAndLoc = await _tipsWarehouseDbContext.Inventories
                .Where(x => x.PartNumber == ItemNumber && x.ProjectNumber == projectNumber && x.LotNumber == lotNumber && x.IsStockAvailable == true).ToListAsync();
