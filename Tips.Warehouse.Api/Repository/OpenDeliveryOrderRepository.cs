...
namespace Tips.Warehouse.Api.Repository
{
    public class OpenDeliveryOrderRepository : RepositoryBase<OpenDeliveryOrder>, IOpenDeliveryOrderRepository
    {
        ...
        public async Task<IEnumerable<OpenDeliveryOrderSPReport>> OpenDeliveryOrderSPReportWithParam(string OpenDONumber, string CustomerName, string CustomerAliasName, string LeadId, 
            string IssuedTo,string KPN, string MPN, string ODOType)
        {

            var result = _tipsWarehouseDbContext
            .Set<OpenDeliveryOrderSPReport>()
            .FromSqlInterpolated($"CALL Open_Delivery_Order_Report_withparameter({OpenDONumber},{CustomerName},{CustomerAliasName},{LeadId},{IssuedTo},{KPN},{MPN},{ODOType})")
            .ToList();
        ...
        public async Task<IEnumerable<OpenDeliveryOrderNoList>> GetAllOpenDeliveryOrderNoList()
        {
            IEnumerable<OpenDeliveryOrderNoList> oDOTypeList = await _tipsWarehouseDbContext.OpenDeliveryOrders
                               .Select(x => new OpenDeliveryOrderNoList()
                               {
                                   Id = x.Id,
